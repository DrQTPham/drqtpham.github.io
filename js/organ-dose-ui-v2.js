...
function calculateCustomOrganDose() {
  const customDose = parseFloat(document.getElementById('customDosePerFraction').value);
  ...
  if (!organ) {
    showAlert('Vui lòng chọn cơ quan trước', 'error');
    resultsDiv.style.display = 'none';
    return;
  }

  // Get all conventional constraints
  if (!organ.conventional || organ.conventional.length === 0) {
    showAlert('Không có dữ liệu conventional cho cơ quan này', 'warning');
    resultsDiv.style.display = 'none';
    return;
  }
